...
Claude Code ハンズオン（60分 × 3回）の教材リポジトリ。ビルド・テスト・依存パッケージは無く、静的 HTML と Markdown のみで構成される。閲覧確認は `open index.html`。
...
- `laravel-prism.html` — 第1回実践教材（Laravel × Prism チャットアプリ構築の実セッション記録）。講義資料とは独立したコンテンツで、session1.html の実践パートから参照される
- `Image/` — スクリーンショット置き場。ファイル名は ASCII セーフ（例: `status.png`, `config-language.png`）を維持する
...
- 各回の時間配分は合計 60 分。セクションの分数を変更したら合計が 60 分になるよう再配分する
- コマンド例は題材（Laravel）に合わせる（`composer.json`、`php artisan test` 等）
