...
#### 4-2. 例：パッケージの install / update をブロック（PreToolUse）（4分）
...
#### 5-3. ハンズオン：公開サンプルを複製してデザイン → コード（4分）
...
* `/agents` で「テスト失敗の原因だけを切り分けるデバッガー」サブエージェントを作り、わざとテストを壊して呼び出してみる

### 7. 実践：Laravel アプリ開発の一連フロー（28分）

**個人開発のフロー** を最初から最後まで回す。Hello World で動作確認 → 題材アプリをクローン → 機能追加（改修） → レビュー → セキュリティレビュー → 修正 → 自分のリポジトリへ push まで、すべて Claude Code との会話で進める。
...
> 💡 CLAUDE.md は **書いたら終わりではなく、生成コードに効いているかを確認** する。守られていない規約があれば、表現を強めるのではなく **理由を添える** 形で書き直す（2-4 のコツ）。

##### 演習6（オプション）：自分の好きな機能を追加する

* チャットアプリに、自分で考えた機能を1つ追加する（例：メッセージの文字数制限、送信履歴の絞り込み、簡単なバリデーション追加など）。Plan Mode で計画 → 承認 → 実装の流れで進める

> ⚠️ **機能を追加したら必ず AI レビューを**：実装が終わったら、次に進む前に **必ず `/code-review`（必要に応じて `/security-review`）で AI レビューを実行** する。自分で考えた機能ほど、バグやセキュリティ観点の見落としが起きやすい。次の 7-5・7-6 はまさにこのレビュー工程にあたる。
...
#### 7-7. レビュー内容を修正して自分のリポジトリへコミット & push（6分）

```
レビュー指摘のうち、○番と○番を直して
```

修正の差分を確認したら、**自分の GitHub リポジトリ** を作成して push する（クローン元は他人のリポジトリなので、そのままでは push できない）:

```
!git add -A && git commit -m "Improve chat app"
# クローン元(他人のorigin)を外し、自分のリポジトリを作成して push
!git remote remove origin
!gh repo create laravel-chat-app --private --source=. --remote=origin --push
```

* `gh repo create --source=. --push` で、新規リポジトリ作成〜push までが会話だけで完結する（初回のみ `gh auth login`）
