...
* **計画の段階で規約が反映されているか** を確認する（例: 履歴のルートに `->name('chat.history')` を付ける計画になっているか、保存処理のバリデーションは FormRequest か）
* 計画をレビュー → 承認 → 実装 → 動作確認（ブラウザでチャットを送り、履歴が表示されるか。`!php artisan test` も可）
...
* 修正の差分を確認する。直した内容が新たな指摘を生んでいないか、必要なら再度 `/code-review` をかける
