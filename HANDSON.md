...
* この例の意味：**`php artisan test` で始まる Bash コマンドは承認なしで自動実行してよい**
* `permissions.allow` … 承認ダイアログを出さずに実行を許可するルールのリスト
* `Bash(...)` … Bash の実行のうち、カッコ内のパターンに一致するもの
* `php artisan test:*` … `:*` は末尾ワイルドカード。`php artisan test --filter=Foo` のように後ろに何が続いても一致する（`php artisan migrate` など別コマンドは対象外）
* よく使う安全なコマンドを登録しておくと、**承認ダイアログの往復が減ってテンポが上がる**（承認時に「Yes, and don't ask again」を選ぶと、ここに自動追記される）
...
第1回で settings.json の配置場所（user / project / local）と `permissions.allow` の基本に触れた。ここでは **書ける主な項目** と、**設定が効いているかの確認方法** を押さえる。
...
| コマンド | 確認できること |
|----------|----------------|
| `/config` | 現在の設定を UI で確認・変更（model, theme, language など） |
...
description: Laravel のコードレビュー専門家。コードを書いた/変更した直後に積極的に使う。
tools: Read, Grep, Glob, Bash
model: sonnet
---

あなたは Laravel のシニアコードレビュアーです。次の観点でレビューしてください：
1. コントローラは薄いか（ロジックはサービスへ）
2. Eloquent の N+1（eager loading で回避）
3. バリデーションは FormRequest を使っているか
4. セキュリティ（SQLインジェクション・CSRF・mass assignment・認可）
5. テストの不足
...
```
● laravel-code-reviewer(app/Http/Controllers の変更をレビュー)
...
**個人開発のフロー** を最初から最後まで回す。Hello World で動作確認 → 題材アプリをクローン → 機能追加（改修） → レビュー → テスト生成 → セキュリティレビュー → 修正 → クローン元リポジトリへの PR 作成まで、すべて Claude Code との会話で進める。
...
> devcontainer 内で起動したサーバー（例: `http://127.0.0.1:8000`）にホスト PC のブラウザからアクセスできない場合は、サーバーの **バインド先** を確認する。
> - **原因**：`--host=127.0.0.1` で起動するとコンテナ内のループバックにしかバインドされず、`docker-compose.yml` に `ports: "8000:8000"` の公開設定があっても、ポートマッピング経由（コンテナのネットワーク IP 宛て）の接続を受け付けられない
> - **対処**：`0.0.0.0` にバインドして起動する — `php artisan serve --host=0.0.0.0 --port=8000`
> - **検証結果（例）**：コンテナ内 `127.0.0.1:8000` → 200 ✅ / コンテナ IP `172.18.0.2:8000` → 200 ✅（ホストからのポートマッピングが通る経路）
...
* レビューは組み込みスキル、テスト生成は自作 `/test-gen` のように、**組み込みでカバーされない定型作業を自作スキルが埋める** 役割分担を意識する

#### 7-7. 組み込みスキルでセキュリティレビュー（4分）
