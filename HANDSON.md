...
* ターミナル / VS Code / JetBrains / Claude Desktop / Web (`claude.ai/code`) の **5つの入口** がある
...
```
[ユーザのプロンプト] → [コンテキスト収集] → [ツール実行] → [結果の検証]
                                ↑________________ ループ ________________|
```

1. ユーザがプロンプトを入れる
2. Claude が必要なコンテキストを集める（ファイル読込・コマンド実行）
3. アクションを実行する（編集・テスト）
4. 結果が要件を満たすか自分で確認 → 満たすまで繰り返す
...
ここまでの内容を使って、実際に Claude Code でアプリを1本作ってみる。題材は PHP の「Hello world」。
...
> 💡 指示が曖昧なとき（CLI か Web か、ファイル名はどうするか等）は Claude が **確認の質問** をしてくることがある。勝手に進めないのが正常な動き。
...
* Plan Mode → 実装 → テスト → コミットの流れで機能追加を完遂できる
...
#### 2-4. Claude Code を用いた個人開発のフロー（3分）

新規の個人プロジェクトを Claude Code 中心で立ち上げる典型的な流れ。**リポジトリの初期化から GitHub への push まで、会話だけで完結できる。**
...
| # | ステップ | 指示・コマンド |
|---|----------|----------------|
| 1 | 雛形の作成 | 「Laravel で TODO アプリの雛形を作って」 |
| 2 | プロジェクトメモリの整備 | `/init` で `CLAUDE.md` を生成 |
| 3 | ローカル Git の初期化 | 「git リポジトリを初期化して、最初のコミットを作って」 |
| 4 | GitHub リポジトリの作成と push | 「gh で GitHub にプライベートリポジトリを作って push して」 |
| 5 | 開発ループ | 機能追加を依頼 → テスト → コミット → push を小さく繰り返す |

ステップ3〜4で Claude が実行するコマンドの例:
...
* `gh`（GitHub CLI）が入っていれば、リポジトリ作成〜push まで Claude に任せられる（初回のみ `gh auth login` で認証が必要）
* コミットメッセージも Claude が **差分から生成** する — 「変更をコミットして」だけでよい
* `/init` で作った `CLAUDE.md` は最初のコミットに含める — 以後のセッション（と将来の自分）への引き継ぎ書になる
...
git diff の内容を確認し、以下の観点でレビューしてください：
1. バグ・エッジケースの見落とし
2. Laravel の規約への準拠
3. セキュリティ上の問題（SQL インジェクション・XSS 等）
指摘は重要度順に、該当ファイル:行番号付きで出力してください。
```

##### 演習2

* 上記の `review` スキルを `.claude/skills/review/SKILL.md` に作成し、`/review` で呼び出す
...
4. 演習2で自作した `/review` と結果を見比べる — 観点の網羅性や出力形式の違いから、**自作スキルを育てるヒント** が得られる
...
題材：チャットアプリに **「モデル選択 UI」を追加** する（Haiku / Sonnet 切り替え）。

#### 5-1. Plan Mode で計画（5分）

`Shift + Tab` で Plan Mode に切り替えてから：

```
チャット画面にモデル選択のドロップダウン（Haiku / Sonnet）を追加したい。
選択したモデルで Prism の呼び出しが行われるようにする。
まず実装計画を立てて。
```

* 出てきた計画を **レビューして赤入れ** する（Plan Mode 中は何度でも安全）

#### 5-2. 実装（10分）

* 計画に納得したら承認して実装開始
* Default モードで **1つ1つの編集を確認しながら** 進める
* エラーが出たら Claude 自身に修正させる — エージェントループを体感する
...
* ブラウザで動作確認 → 問題があれば会話で修正指示
* 完了したら Claude にコミットメッセージを書かせてコミット
...
* Claude Code を組み込んだ PR ベースの開発フローを回せる
...
題材：チャットアプリへの小さな機能追加を **PR ベース** で行う。

#### 5-1. ブランチ作成 〜 実装（7分）

```
会話履歴を保持してマルチターンの会話ができるようにしたい。
feature ブランチを切って、Plan Mode で計画してから実装して。
```

* ブランチ作成・実装・テストまで Claude に任せ、人間は **計画と差分のレビュー** に集中する
...
* `gh` コマンドで PR が作成される — 説明文も Claude が差分から生成する
* 受講者同士で PR を交換し、`/review` スキルでレビューしてみる

##### `claude --from-pr` — PR の文脈で再開

```bash
claude --from-pr <N>   # N は PR 番号
```

* PR 番号を指定して起動すると、その PR の **差分・説明文・レビューコメント** を読み込んだ状態でセッションが始まる
* レビュー指摘への対応が一気に楽になる — 「レビューコメントに対応して」のひと言で修正が始められる
* PR を作った本人でなくても、**PR の背景を把握した状態** でレビューや修正を引き継げる — チーム開発でこそ効く

###### 実践の流れ

1. レビュアーから PR に指摘コメントが付く
2. `claude --from-pr <N>` で起動 — PR の文脈が読み込まれる
3. 「レビューコメントに対応して」と指示 → Claude が指摘内容を読み、修正 → push まで実行
4. 対応結果を PR 上のコメントで報告させる
